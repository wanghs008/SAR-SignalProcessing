...
subplot(322),imagesc( abs(soutt_1))
%  axis([10 150,0 256])
xlabel('距离向(采样点)→'),ylabel('←方位向(采样点)'),title('(b)幅度')
%  sgtitle('图6.4 方式一得到距离压缩数据','Fontsize',16,'color','k')
subplot(323),imagesc(real(soutt_2))
%  axis([10 150,0 256])
xlabel('距离向(采样点)→'),ylabel('←方位向(采样点)'),title('(a)实部')
subplot(324),imagesc( abs(soutt_2))
%  axis([10 150,0 256])
xlabel('距离向(采样点)→'),ylabel('←方位向(采样点)'),title('(b)幅度')
%  sgtitle('图6.4 方式二得到距离压缩数据','Fontsize',16,'color','k')
subplot(325),imagesc(real(soutt_3))
%  axis([10 150,0 256])
xlabel('距离向(采样点)→'),ylabel('←方位向(采样点)'),title('(a)实部')
subplot(326),imagesc( abs(soutt_3))
%  axis([10 150,0 256])
xlabel('距离向(采样点)→'),ylabel('←方位向(采样点)'),title('(b)幅度')
sgtitle('图6.4 距离压缩数据','Fontsize',16,'color','k')